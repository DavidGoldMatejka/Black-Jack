...
class BlackJack{
  ...
  private:
    Stack deck;           // source deck of cards 
    Stack dealerHand;     // dealer's "hand" 
    Stack playerHand;    // player's hand
  ...
  public:
    BlackJack(     // constructor
          std::vector<std::string> _playerNames = {"Player", "Dealer"},  // the characters that describe the players, defaults to X and O
          std::vector<int> cardID = {} // vector of card IDs we want in the  deck; defaults to empty (player and dealer hands will be empty)
       ); // PROTOTYPE!
    ...
    int getPlayerPoints(); // return the points for the player's hand by calling Stack::playerHand->getPoints()
    int getDealerPoints(); // return the points for the dealer's hand by calling Stack::dealerHand->getPoints()
